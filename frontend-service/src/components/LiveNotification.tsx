...
export default function LiveNotification() {
    ...
    return (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[9999] flex flex-col items-center space-y-4 pointer-events-none w-full max-w-2xl px-4">
            {notifications.map((notif) => {
                ...
                return (
                    <div 
                        key={notif.id}
                        className={`flex items-start w-full p-4 rounded-2xl shadow-2xl backdrop-blur-md border animate-in slide-in-from-top-4 fade-in duration-500
                    ...
                    >
                        ...
                        <div className="flex-1 pt-1">
                            <p className="text-sm font-bold tracking-widest uppercase opacity-70 mb-1">
                                {isInfo ? 'System Broadcast' : isSuccess ? 'Success' : isWarning ? 'Warning' : 'Critical Alert'}
                            </p>
                            <p className="text-base sm:text-lg lg:text-xl font-medium leading-relaxed shadow-sm drop-shadow-md">
                                {notif.message}
                            </p>
                        </div>
                    </div>
